...
// Same `lms-screen/required-courses` endpoint as My Required Courses —
// confirmed against `origin/staging`'s MyRequiredCoursesController::
// actionIndex(), which serves both "My Required Courses" and "My
// Recommended Courses" off the same action, differing only by the
// `type` query param (`required` vs `recommended`) and page title.
...
class RecommendedCoursesViewModel
    extends StateNotifier<RecommendedCoursesState> {
  RecommendedCoursesViewModel({required this.repository, required this.userId})
    : super(const RecommendedCoursesState()) {
    fetch();
  }
