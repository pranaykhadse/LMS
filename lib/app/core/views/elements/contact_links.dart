...
/// with auto-login redirect so the session carries over.
Future<void> launchContactCoachUrl(WidgetRef ref, BuildContext context) async {
  ...
  // This URL already embeds auth credentials in query params — no need for
  // the redirect-login-link step; open directly in the in-app WebView.
  await InAppWebViewPage.show(context, url: url, title: 'Contact a Coach');
}

Future<void> launchVirtualDevUrl(BuildContext context, WidgetRef ref) async {
  const url =
  ...
  await InAppWebViewPage.showWithAuth(context, ref,
      url: url, title: 'Virtual Development Pro');
}
